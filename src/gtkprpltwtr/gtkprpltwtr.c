...
static void twitter_conv_id_write_message(PurpleAccount * account, TwitterConversationId * conv_id, PurpleMessageFlags flags, const gchar * message)
{
...
static void twitter_send_rt_error_cb(TwitterRequestor * r, const TwitterRequestErrorData * error_data, gpointer user_data)
{
    TwitterConversationId *conv_id = user_data;
...
static void twitter_add_favorite_error_cb(TwitterRequestor * r, const TwitterRequestErrorData * error_data, gpointer user_data)
{
    TwitterConversationId *conv_id = user_data;
...
static void twitter_delete_favorite_error_cb(TwitterRequestor * r, const TwitterRequestErrorData * error_data, gpointer user_data)
{
    TwitterConversationId *conv_id = user_data;
...
static void twitter_report_spammer_error_cb(TwitterRequestor * r, const TwitterRequestErrorData * error_data, gpointer user_data)
{
    TwitterConversationId *conv_id = user_data;
...
static void twitter_get_status_success_cb(TwitterRequestor * r, xmlnode * node, gpointer user_data)
{
    ...
    if (!status || !status->text || !status->id) {
        purple_debug_error(DEBUG_ID, "Essential information missing from the tweet!\n");
        return;
    }

    user = twitter_user_node_parse(xmlnode_get_child(node, "user"));
    if (!user || !user->screen_name) {
        purple_debug_error(DEBUG_ID, "Essential information missing from the user!\n");
        return;
    }
...
static void twitter_get_status_error_cb(TwitterRequestor * r, const TwitterRequestErrorData * error_data, gpointer user_data)
{
    TwitterConversationId *conv_id = user_data;
...
static void twitter_delete_tweet_error_cb(TwitterRequestor * r, const TwitterRequestErrorData * error_data, gpointer user_data)
{
    TwitterConversationId *conv_id = user_data;
...
static void gtkprpltwtr_mark_reply(PurpleConversation * conv, const gchar * id_str)
{
    ...
    if (!reply_icon) {
        purple_debug_error(DEBUG_ID, "Couldn't load reply icon!\n");
        return;
    }
...
static gboolean twitter_uri_handler(const char *proto, const char *cmd_arg, GHashTable * params)
{
    ...
    if (username == NULL || username[0] == '\0') {
        purple_debug_error(DEBUG_ID, "malformed uri. No account username\n");
        return FALSE;
    }
    ...
    if (account == NULL) {
        purple_debug_error(DEBUG_ID, "could not find account %s\n", username);
        return FALSE;
    }
    ...
    if (!strcmp(cmd_arg, TWITTER_URI_ACTION_USER)) {
        ...
        if (id_str == NULL || user == NULL || id_str[0] == '\0' || user[0] == '\0' || text == NULL || text[0] == '\0') {
            purple_debug_error(DEBUG_ID, "malformed uri. Invalid id/user for reply\n");
            return FALSE;
        }
        id = strtoll(id_str, NULL, 10);
        if (id == 0) {
            purple_debug_error(DEBUG_ID, "malformed uri. Invalid id for reply\n");
            return FALSE;
        }
        ...
        if (id_str == NULL || id_str[0] == '\0') {
            purple_debug_error(DEBUG_ID, "malformed uri. Invalid id for rt\n");
            return FALSE;
        }
        id = strtoll(id_str, NULL, 10);
        if (id == 0) {
        ...
        if (in_reply_to_status_id_str == NULL || in_reply_to_status_id_str[0] == '\0') {
            purple_debug_error(DEBUG_ID, "malformed uri. Invalid in_reply_to_status_id_str for GET_ORIGINAL\n");
            return FALSE;
        }
        in_reply_to_status_id = strtoll(in_reply_to_status_id_str, NULL, 10);
        if (in_reply_to_status_id == 0) {
            purple_debug_error(DEBUG_ID, "malformed uri. Invalid in_reply_to_status_id for GET_ORIGINAL \n");
            return FALSE;
        }
        ...
        TwitterConnectionData *twitter;
        if (!gc) {
            purple_debug_warning(DEBUG_ID, "disconnected. Exiting\n.");
            return FALSE;
        }
        twitter = gc->proto_data;
        id_str = g_hash_table_lookup(params, "id");
        user = g_hash_table_lookup(params, "user");
        if (id_str == NULL || user == NULL || id_str[0] == '\0' || user[0] == '\0') {
            purple_debug_error(DEBUG_ID, "malformed uri. Invalid id/user for link\n");
            return FALSE;
        }
        id = strtoll(id_str, NULL, 10);
        if (id == 0) {
            purple_debug_error(DEBUG_ID, "malformed uri. Invalid id for link\n");
            return FALSE;
        }
        ...
        if (text == NULL || text[0] == '\0') {
            purple_debug_error(DEBUG_ID, "malformed uri. No text for search\n");
            return FALSE;
        }
        ...
        id_str = g_hash_table_lookup(params, "id");
        if (id_str == NULL || id_str[0] == '\0') {
            purple_debug_error(DEBUG_ID, "malformed uri. Invalid id for favoriting\n");
            return FALSE;
        }
        id = strtoll(id_str, NULL, 10);
        if (id == 0) {
        ...
        user = g_hash_table_lookup(params, "user");
        if (user == NULL || user[0] == '\0') {
            purple_debug_error(DEBUG_ID, "malformed uri. Invalid user for marking as spam\n");
            return FALSE;
        }
        conv_id = g_new0(TwitterConversationId, 1);
        conv_id->conv_name = g_strdup(purple_url_decode(conv_name_encoded));
        conv_id->type = conv_type;
        twitter_api_report_spammer(purple_account_get_requestor(account), user, twitter_report_spammer_success_cb, twitter_report_spammer_error_cb, conv_id);
    }
    return TRUE;
}
...
static gboolean twitter_url_clicked_cb(GtkIMHtml * imhtml, GtkIMHtmlLink * link)
{
    ...
    menu = gtk_menu_new();
    g_object_set_data_full(G_OBJECT(menu), "x-imhtml-url-data", url, (GDestroyNotify) g_free);
...
static void gtkprpltwtr_enable_conv_icon_all_accounts()
{
    if (purple_prefs_get_bool(TWITTER_PREF_ENABLE_CONV_ICON)) {
        ...
        for (l = accounts; l; l = l->next) {
            ...
            if (purple_account_is_connected(account) && (!strcmp(TWITTER_PROTOCOL_ID, purple_account_get_protocol_id(account)) || !strcmp(STATUSNET_PROTOCOL_ID, purple_account_get_protocol_id(account))
                )
                ) {
                twitter_conv_icon_account_load(account);
            }
...
{
    purple_signal_connect(purple_buddy_icons_get_handle(), "prpltwtr-update-buddyicon", gtkprpltwtr_plugin, PURPLE_CALLBACK(gtkprpltwtr_update_buddyicon_cb), NULL);
    purple_signal_connect(purple_buddy_icons_get_handle(), "prpltwtr-update-iconurl", gtkprpltwtr_plugin, PURPLE_CALLBACK(gtkprpltwtr_update_iconurl_cb), NULL);
    gtkprpltwtr_enable_conv_icon_all_accounts();
}

static void gtkprpltwtr_disable_conv_icon()
{
    purple_signal_disconnect(purple_buddy_icons_get_handle(), "prpltwtr-update-buddyicon", gtkprpltwtr_plugin, PURPLE_CALLBACK(gtkprpltwtr_update_buddyicon_cb));
    purple_signal_disconnect(purple_buddy_icons_get_handle(), "prpltwtr-update-iconurl", gtkprpltwtr_plugin, PURPLE_CALLBACK(gtkprpltwtr_update_iconurl_cb));
    gtkprpltwtr_disable_conv_icon_all_accounts();
}

static void gtkprpltwtr_pref_enable_conv_icon_change(const char *name, PurplePrefType type, gconstpointer val, gpointer data)
{
    if (purple_prefs_get_bool(TWITTER_PREF_ENABLE_CONV_ICON)) {
...
static gboolean plugin_load(PurplePlugin * plugin)
{
    ...
    purple_signal_connect(purple_conversations_get_handle(), "conversation-created", plugin, PURPLE_CALLBACK(twitter_charcount_conv_created_cb), NULL);
    purple_signal_connect(purple_conversations_get_handle(), "deleting-conversation", plugin, PURPLE_CALLBACK(twitter_charcount_conv_destroyed_cb), NULL);

    purple_signal_connect(purple_accounts_get_handle(), "prpltwtr-connecting", plugin, PURPLE_CALLBACK(gtkprpltwtr_connecting_cb), NULL);

    purple_signal_connect(purple_accounts_get_handle(), "prpltwtr-disconnected", plugin, PURPLE_CALLBACK(gtkprpltwtr_disconnected_cb), NULL);

    if (purple_prefs_get_bool(TWITTER_PREF_ENABLE_CONV_ICON))
        gtkprpltwtr_enable_conv_icon();

#if PURPLE_VERSION_CHECK(2, 6, 0)
    purple_signal_connect(purple_conversations_get_handle(), "prpltwtr-format-tweet", plugin, PURPLE_CALLBACK(gtkprpltwtr_format_tweet_cb), NULL);

    purple_signal_connect(purple_conversations_get_handle(), "prpltwtr-received-im", plugin, PURPLE_CALLBACK(gtkprpltwtr_received_im_cb), NULL);

    purple_signal_connect(purple_conversations_get_handle(), "prpltwtr-set-reply", plugin, PURPLE_CALLBACK(gtkprpltwtr_mark_reply), NULL);

    purple_signal_connect(purple_get_core(), "uri-handler", plugin, PURPLE_CALLBACK(twitter_uri_handler), NULL);

    purple_signal_connect(purple_conversations_get_handle(), "prpltwtr-changed-attached-search", plugin, PURPLE_CALLBACK(twitter_charcount_update_append_text_cb), NULL);
    ...
    purple_prefs_connect_callback(plugin, TWITTER_PREF_ENABLE_CONV_ICON, gtkprpltwtr_pref_enable_conv_icon_change, NULL);
...
static PurplePluginPrefFrame *get_plugin_pref_frame(PurplePlugin * plugin)
{
    ...
    ppref = purple_plugin_pref_new_with_name_and_label(TWITTER_PREF_ENABLE_CONV_ICON, _("Enable Icons in Chat"));
