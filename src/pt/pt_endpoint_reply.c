...
static int twitter_send_reply_do(PurpleAccount * account, const char *who, const char *message, PurpleMessageFlags flags)
{
    ...
    if (!in_reply_to_status_id) {
        ...
        reply_id = (const gchar *) g_hash_table_lookup(twitter->user_reply_id_table, who);
        if (reply_id)
...
static void _process_replies(PurpleAccount * account, GList * statuses, TwitterConnectionData * twitter)
{
    ...
    for (l = statuses; l; l = l->next) {
        ...
        if (!user_data) {
            ...
            pt_endpoint_im_update_conv(ctx, data->screen_name, status);
...
static void twitter_get_replies_last_since_id(PurpleAccount * account, void (*success_cb) (PurpleAccount * account, long long id, gpointer user_data), void (*error_cb) (PurpleAccount * account, const TwitterRequestErrorData * error_data, gpointer user_data), gpointer user_data)
{
...
PurpleConversation *twitter_endpoint_reply_conversation_new(TwitterEndpointIm * im, const gchar * buddy_name, long long reply_id, gboolean force)
{
    if (im) {
        ...
        purple_debug_info(purple_account_get_protocol_id(im->account), "%s() conv %p (%s) %s replies to %lld\n", G_STRFUNC, conv, conv_name, force ? "force" : "suggest", reply_id);
